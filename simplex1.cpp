...
void findMaxModuleElement(const Matrix & x, std::size_t & column_index ) //number of colum with abs max value
{
	...
	std::size_t col= x.getCol() - 1;
	Element_type maxModule = 0;
	for( std::size_t j = 0; j < col; ++j)
	{
		if( x.getElement(row,j) < 0 &&  ( myAbs(maxModule) <  myAbs(x.getElement( row,j))))
		{
	...
	std::cout <<"column index = " <<  column_index<< std::endl;//test
	std::cout << "maxModule =" << maxModule <<std::endl;//test
}
...
void divideFreeMember(Matrix &  x_before, std::size_t & column_index, std::size_t & row_index)
{
	...
	std::size_t row = x_before.getRow()-1;
	...
	for(std::size_t i = 1; i< row; ++i)
	{
		...
		std::cout << "temp1 = " << temp1 << "column index = " << column_index << std::endl;//test
}
...
void findMinColumnElement(Matrix & x, std::size_t&  row_index)
{
	...
	std::cout <<"\n " << "value = " <<  value << " " <<"row_index = " <<  row_index << std::endl; //test
}
...
void dividePivotColumn( Matrix & x,  std::size_t & row , std::size_t & col)
{
	...
	std::cout << "dividePivotColunm = " << temp  << std::endl;
}
...
void findMaxValue( Matrix & x , Element_type & max)
{
	std::cout << "Max value = " << x.getElement(x.getRow() -1 ,x.getCol() -1)<< std::endl;
	std::cout << "x1 = " << x.getElement( x.getRow()-2 , x.getCol() - 1)<< std::endl;
	std::cout << "x2 = " << x.getElement( x.getRow()-3 , x.getCol() - 1) << std::endl;
	std::cout << "s = " << x.getElement( x.getRow()-4 , x.getCol() - 1) << std::endl;
}

void simplexMethod(const  Matrix & a, const  Matrix &b, Matrix &x)
{
	std::size_t col = x.getCol();
	std::size_t row = x.getRow()-1;
